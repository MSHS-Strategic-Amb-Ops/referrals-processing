...
pt_referrals <- referral_vol %>%
  filter(str_detect(ProcedureName, "PHYSICAL THERAPY"))

require(openxlsx)
write.csv(pt_referrals, "PT Referrals.csv")

pt_summary <- pt_referrals %>%
  group_by(createdYear, createdMonNum, createdMonthYear, ProcedureName, 
           SentBySite, SentBySpecialty, SentByDept, SentByDepID, 
           ReceivedBySite, ReceivedBySpecialty, ReceivedByDept, ReceivedByDepID,
           ApptStatusFinal, Status,
           ICD_10_Chosen, FIN_CLASS_NAME)
